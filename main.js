// HTML & CSS for this project is made by https://codepen.io/merkund/pen/rQjKGj?page=4, all credits due

// Feture Wishlist
// Display used operator along with number in UI
...
class Calc {
    ...
    static operate(input, operator) {
        ...
        // Addresses User Switching Operator in Between Calculation, ie for input 2+*5 will return 2*5;
            ...
            if (!(operator === 'equate')) {
                // Updating Operator Memory to Newly Selected Operator for Next Calculation
                Calc.operatorMemory = operator;
                Calc.equated = false;
            } else {
            ...
            if (e.target.className === 'numpad') {
                ...
                if (/\./.test(e.target.textContent)) {
                    ...
                    if (!(/\./.test(Event.inputString))) {
                        ...
                        if(Event.inputString.length < UI.displayLimit){Event.inputString += e.target.textContent}
                    }
